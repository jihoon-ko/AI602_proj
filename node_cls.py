...
def main(args):
    ...
    model = GCN(dataset.num_node_features, args.hidden_dim, dataset.num_classes, args.num_layers, D=args.D, L=args.L, K=args.K, model_type=args.model_type).to(device)
    print(model)
    ...
    for epoch in range(args.num_epochs):
        model.train()
        optimizer.zero_grad()
        out, reg = model(data, args.tau)
        loss = loss_fn(out[data.train_mask], data.y[data.train_mask]) + args.lmbda * reg
        loss.backward()
        optimizer.step()
        
        model.eval()
        pred = model(data, args.tau)[0].argmax(dim=1)
    ...
    with open('log_data_{}_type_{}.txt'.format(args.dataset, args.model_type), 'a') as f:
        f.write('dim_{}_nl_{}_L_{}_K_{}_D_{}_tau_{}_lmbda_{}'.format(args.hidden_dim, args.num_layers, args.L, args.K, args.D, args.tau, args.lmbda) + '\n')
        f.write(str(max_acc) + '\n\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num-layers", type=int, default=5, help="number of the layers")
    ...
    parser.add_argument("--L", type=int, default=2, help="number of levels")
    parser.add_argument("--K", type=int, default=32, help="size of cluster 1")
